...
// Импортируем иконки из lucide-react и react-icons
      ...
      <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b">
          {viewAll ? (
            ...
            <button className="p-2 opacity-0">{/* placeholder */}</button>
          )}
        ...
        {!viewAll ? (
          <div className="px-4 py-3 space-y-4">
            ...
            <button
              onClick={() => handleConnect({ jsBridgeKey: "tonkeeper", injected: false })}
            ...
            <div className="grid grid-cols-4 gap-4">
              {primary.map((w) => (
                <button
                  ...
                  className="flex flex-col items-center space-y-1 hover:bg-gray-50 p-2 rounded-lg"
                >
                  <img src={w.imageUrl} alt={w.name} className="w-10 h-10 rounded-full" />
          ...
          <div className="px-4 py-3 bg-gray-50 max-h-[60vh] overflow-y-auto">
            <div className="grid grid-cols-4 gap-4">
              {wallets.map((w) => (
                <button
                  key={w.name}
                  onClick={() => handleConnect(w)}
                  className="flex flex-col items-center space-y-1 hover:bg-white p-2 rounded-lg"
                >
                  <img src={w.imageUrl} alt={w.name} className="w-10 h-10 rounded-full" />
                  <span className="text-xs text-center text-gray-900">{w.name}</span>
                  {connecting === w.name && (
